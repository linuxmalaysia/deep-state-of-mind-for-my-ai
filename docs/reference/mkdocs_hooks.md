...
The `mkdocs_hooks.py` script intercepts static pages during compilation. It strips unnecessary `docs/` prefixes and normalizes repository-root relative paths (`../../` to `../`), ensuring links work seamlessly on both GitHub.com and built HTML servers.
...
- **External links:** Keeps `http://`, `https://`, `mailto:`, and `#` anchor links unchanged.
- **`docs/` prefix:** Strips `docs/` from relative paths (e.g. `docs/governance/PROTOCOL.md` becomes `governance/PROTOCOL.md`).
- **`../../` prefix:** Normalizes double-parent directories (e.g. `../../AGENTS.md` becomes `../AGENTS.md`).

...
2. **Convert to HTML:** Convert the markdown into standard, clean, semantic HTML (`<h2>`, `<p>`, `<ul>`, etc.). Do not apply heavy inline CSS, as the Blogger theme will handle presentation. Wrap the entire post in `<div class="dsom-post">`.
3. **Inject the Collaboration Note (TOP):** At the very beginning of the post, insert the following blockquote exactly:
   ```html
   ...
   ```
4. **Inject Repository Links (BOTTOM):** At the end of the post, immediately before the signature, insert the open-source repository links:
   ```html
   ...
   ```
5. **Output Delivery:** Output the fully assembled HTML string inside a standard markdown code block in the chat, so the user can easily copy and paste it into the Blogger HTML view.

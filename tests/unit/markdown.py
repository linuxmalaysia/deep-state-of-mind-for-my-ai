    ...
    def test_markdown_okf_compliance(self):
        ...
        for md_file in md_files:
            with self.subTest(file=md_file.relative_to(self.repo_root).as_posix()):
                ...
                # Verify string fields (type, title, timestamp) with special characters/emojis use double-quoted scalar style
                for req_key in ("type", "title", "timestamp"):
                ...
                # Verify each topics entry double-quoting for special characters
                if "topics" in node_map:
                    topics_node = node_map["topics"]
                    if isinstance(topics_node, yaml.SequenceNode):
                        for item_node in topics_node.value:
                            if isinstance(item_node, yaml.ScalarNode):
                                item_str = str(item_node.value)
                                if any(ord(c) > 127 for c in item_str) or any(c in item_str for c in ":[]()"):
                                    self.assertEqual(
                                        item_node.style, '"',
                                        f"Topic item '{item_str}' in {md_file.name} contains special characters and must be double-quoted"
                                    )
